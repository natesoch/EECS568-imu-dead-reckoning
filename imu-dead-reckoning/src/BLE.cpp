...
//     // create a buffer for the size of the message + 1 byte for message type + 1 bytes for length
//     uint8_t totalSize =sizeof(MessageType);
...
//     // send the data and notify
//     if(deviceConnected){
//         xSemaphoreTake(xbleMutex, portMAX_DELAY);
//         pServerTxCharacteristic->setValue(buffer, totalSize);
//         pServerTxCharacteristic->notify();
