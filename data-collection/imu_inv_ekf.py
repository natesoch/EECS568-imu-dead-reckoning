...
def ahrs_riekf(iekf_filter, data):
    ...
    accel_std = pd.Series(accel_mag).rolling(window=15, center=True).std().fillna(0).values #TODO: play around with window size, 15 is nice for now
    ...
    for i in range(N):
        if dt[i] > 0:
            ...
            if is_stationary:
                ...
                v = v * 0.3 #TODO: play around with this 0.3 is nice for now 
                a_world = np.zeros(3) 
            else:
                v = v + (a_world * dt[i])
